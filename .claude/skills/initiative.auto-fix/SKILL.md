...
You are a non-interactive Initiative auto-fix pipeline. Do not ask questions or wait for confirmation. Make all decisions autonomously.
...
- Remaining arguments: explicit Initiative IDs

### 1. Init

```bash
python3 scripts/pipeline_state.py init --type initiative [--batch-size N] [--headless] [--announce-complete]
```
...
```bash
python3 scripts/snapshot_fetch.py fetch "<query>" --type initiative --ids-file tmp/pipeline-all-ids.txt --changed-file tmp/pipeline-changed-ids.txt [--limit N] [--data-dir "<path>"] [--reprocess] [--random N]
```
...
```bash
python3 scripts/snapshot_fetch.py fetch --reprocess --type initiative --ids-file tmp/pipeline-all-ids.txt --changed-file tmp/pipeline-changed-ids.txt
```
...
agents:
  - subagent_type: initiative-scorer
    ...
    vars: |
      ...
      PROMPT_PATH=.context/assess-rfe/skills/assess-initiative/scripts/agent_prompt.md
  - prompt_file: .claude/skills/initiative-feasibility-review/SKILL.md
    vars: |
      ID=RHOAIENG-1234
  - prompt_file: .claude/skills/strategic-alignment-review/SKILL.md
    vars: |
      ID=RHOAIENG-1234
```
...
```bash
python3 scripts/batch_summary.py --type initiative --counts-only --ids-file tmp/pipeline-all-ids.txt
```
