...
def _latest_snapshot(work_dirs):
    ...
    snaps = sorted(
        [f for f in os.listdir(work_dirs.snapshot_dir) if f.startswith("issue-snapshot-")],
        reverse=True,
    )
...
def _seed_snapshot(work_dirs, issues, query_ts="2026-04-01T00:00:00Z"):
    ...
    path = os.path.join(work_dirs.snapshot_dir, "issue-snapshot-20260401-000000.yaml")
    with open(path, "w") as f:
...
class TestCmdFetchFirstRun:
    ...
    def test_snapshot_written(self, work_dirs, jira, monkeypatch, tmp_path):
        ...
        snaps = [f for f in os.listdir(work_dirs.snapshot_dir) if f.startswith("issue-snapshot-")]
        assert len(snaps) == 1
        with open(os.path.join(work_dirs.snapshot_dir, snaps[0])) as f:
...
class TestCmdFetchWithDataDir:
    def _make_data_dir(self, tmp_path, snapshot_issues):
        ...
        run_dir = os.path.join(data_dir, "20260401-120000", "auto-fix-runs")
        os.makedirs(run_dir)
        ...
        snap_path = os.path.join(run_dir, "issue-snapshot-20260401-120000.yaml")
    ...
    def test_falls_back_to_data_dir(self, work_dirs, jira, monkeypatch, tmp_path):
        """No local snapshot → uses data-dir snapshot for diffing."""
        jira.create("RHAIRFE-1", "Issue one", "Changed content.")
        data_dir = self._make_data_dir(tmp_path, {"RHAIRFE-1": "old-hash"})
        _jira_env(monkeypatch, jira.url)
    ...
    def test_test_data_dir_skipped(self, work_dirs, jira, monkeypatch, tmp_path):
        ...
        with open(os.path.join(td_run_dir, "issue-snapshot-20260401-120000.yaml"), "w") as f:
...
class TestMultiRunPipeline:
    ...
    def test_steady_state_skips_unchanged(self, work_dirs, jira, monkeypatch, tmp_path):
        ...
        update_snapshot_hashes(
            {}, work_dirs.snapshot_dir, mark_processed=["RHAIRFE-1", "RHAIRFE-2"]
        )
    ...
    def test_user_edits_after_submit(self, work_dirs, jira, monkeypatch, tmp_path):
        ...
        revised_hash = compute_content_hash(_text_to_adf("Auto-revised description."))
        update_snapshot_hashes(
            {"RHAIRFE-1": revised_hash}, work_dirs.snapshot_dir, mark_processed=["RHAIRFE-2"]
        )
        # Simulate Jira now has the revised description
        jira.request(
            "PUT",
            "/rest/api/3/issue/RHAIRFE-1",
        ...
        jira.request(
            "PUT",
            "/rest/api/3/issue/RHAIRFE-1",
            {"fields": {"description": "User rewrote this after our fix."}},
        )
    ...
    def test_submit_without_user_edit(self, work_dirs, jira, monkeypatch, tmp_path):
        ...
        # Run 1: submit revises RHAIRFE-1, updates snapshot
        revised_hash = compute_content_hash(_text_to_adf("We revised this."))
        update_snapshot_hashes({"RHAIRFE-1": revised_hash}, work_dirs.snapshot_dir)
        # Simulate Jira now has our revision
        jira.request(
            "PUT", "/rest/api/3/issue/RHAIRFE-1", {"fields": {"description": "We revised this."}}
        )
    ...
    def test_new_issue_created_by_submit(self, work_dirs, jira, monkeypatch, tmp_path):
        ...
        new_hash = compute_content_hash(_text_to_adf("We created this."))
        update_snapshot_hashes({"RHAIRFE-2": new_hash}, work_dirs.snapshot_dir)
        ...
        jira.request(
            "PUT",
            "/rest/api/3/issue/RHAIRFE-2",
            {"fields": {"description": "User improved our new issue."}},
        )
    ...
    def test_issue_leaves_scope(self, work_dirs, jira, monkeypatch, tmp_path):
        ...
        transitions = jira.request("GET", "/rest/api/3/issue/RHAIRFE-2/transitions")
        done_t = next(
            (t for t in transitions["transitions"] if t["to"]["statusCategory"]["key"] == "done"),
            None,
        )
        if done_t:
            jira.request(
    ...
    def test_mixed_activity_across_runs(self, work_dirs, jira, monkeypatch, tmp_path):
        ...
        revised_hash = compute_content_hash(_text_to_adf("Revised issue one."))
        update_snapshot_hashes(
            {"RHAIRFE-1": revised_hash}, work_dirs.snapshot_dir, mark_processed=["RHAIRFE-2"]
        )
        jira.request(
            "PUT", "/rest/api/3/issue/RHAIRFE-1", {"fields": {"description": "Revised issue one."}}
        )
        ...
        jira.request(
            "PUT",
            "/rest/api/3/issue/RHAIRFE-2",
            {"fields": {"description": "User rewrote issue two."}},
        )
...
class TestCumulativeSnapshot:
    ...
    def test_new_persists_until_selected(self, work_dirs, jira, monkeypatch, tmp_path):
    ...
    def test_closed_issue_persists_in_snapshot(self, work_dirs, jira, monkeypatch, tmp_path):
    ...
    def test_stale_hash_detects_edit_on_unselected(self, work_dirs, jira, monkeypatch, tmp_path):
    ...
    def test_no_limit_includes_all_in_snapshot(self, work_dirs, jira, monkeypatch, tmp_path):
        ...
        snap = _latest_snapshot(work_dirs)
        assert len(snap["issues"]) == 3
        assert set(snap["issues"].keys()) == {"RHAIRFE-1", "RHAIRFE-2", "RHAIRFE-3"}
    ...
    def test_post_submit_hash_composes_with_cumulative_merge(
        ...
        jira.request(
            "PUT", "/rest/api/3/issue/RHAIRFE-1", {"fields": {"description": "Revised by submit."}}
        )
...
class TestCloneThenFetch:
    ...
    def test_clone_provides_baseline_for_incremental_fetch(self, jira, monkeypatch, tmp_path):
        ...
        existing_hash = compute_content_hash(_text_to_adf("Already processed."))
        snap = yaml.dump(
            {
                "query_timestamp": "2026-04-01T00:00:00Z",
                "timestamp": "2026-04-01T00:00:01Z",
        ...
        _git_add_commit(
            src,
            "20260401-120000/auto-fix-runs/issue-snapshot-20260401-120000.yaml",
            snap,
            "add snapshot",
        )
